/**
 * Discord Role Bot – buttons + reaction roles
 * Features: single/multi select, target channel, reaction auto-sync on startup + /rr sync
...
async function syncReactionPanel(guild, messageId, entry) {
  ...
  if (!channelId) return; // old data with no channel reference
  ...
  for (const [roleId, userSet] of wantRoleUsers.entries()) {
    for (const userId of userSet) {
      ...
      if (exclusive) {
        // keep only one role according to the user's current reaction
        ...
        // multi-select: make sure they have all roles they reacted to
  ...
  // Optional cleanup: remove panel roles from users who no longer react at all
  try {
...
client.once('ready', async () => {
  ...
  // auto-sync every guild on startup
...
// Button handler (for /rr create panels)
client.on('interactionCreate', async interaction => {
